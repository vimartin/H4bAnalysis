...
int main(int argc, char** argv){
  ...
  // Parameters
  if (argc != 2) {
    cout << "bmuon.exe [configFile]" << endl;
    return 0;
  }
  INIReader reader("JOs/bmuon_analysis.cfg");
  if (reader.ParseError() < 0) {
    std::cout << "Can't load JOs/bmuon_analysis.cfg" << endl;
    return 1;
  }
  ...
  std::string outputFileName = Form("bmuon_%s", reader_sample.Get("io", "result_file_name", "UNKNOWN").c_str());
  ...
  cutflow_bin_title.push_back(">= 1 mu");
  cutflow_bin_title.push_back(">= 3 mu");
  cutflow_bin_title.push_back("mu pt");
  cutflow_bin_title.push_back("mu eta");
  cutflow_bin_title.push_back("mu iso");
...
//    std::cout<<counter<<std::endl;
    ...
    // Find the index of the final state muons
    std::vector<int> theMuons = finalStateMuons(event->m_genParticles, isSherpa, isPythia6);
    ...
    for (auto part : event->m_genParticles) {
      ...
      // Get TLorentz vector of the muon
      if (std::find(theMuons.begin(), theMuons.end(), index)!=theMuons.end()) {
        ...
        plot1D("h_muon_preselected_pt", partvec.Pt()/1000., 1., h_1d, "pre-selected muon pT", 100, 0, 150);
        plot1D("h_muon_preselected_eta", partvec.Eta(), 1., h_1d, "pre-selected muon eta", 100, -5, 5);
        continue;
      }

      // dRmin between muons
      float dRmin = 100.;
      if (pre_selected_lepton.size()==2){
        TLorentzVector lep1 = pre_selected_lepton.at(0).getLeptonRaw();
        TLorentzVector lep2 = pre_selected_lepton.at(1).getLeptonRaw();
        dRmin = lep1.DeltaR(lep2);
      }
      if (pre_selected_lepton.size()==3){
        TLorentzVector lep1 = pre_selected_lepton.at(0).getLeptonRaw();
        TLorentzVector lep2 = pre_selected_lepton.at(1).getLeptonRaw();
        TLorentzVector lep3 = pre_selected_lepton.at(2).getLeptonRaw();
        dRmin = min(min(lep1.DeltaR(lep2), lep1.DeltaR(lep3)), lep2.DeltaR(lep3));
      }
      if (pre_selected_lepton.size()==4){
        TLorentzVector lep1 = pre_selected_lepton.at(0).getLeptonRaw();
        TLorentzVector lep2 = pre_selected_lepton.at(1).getLeptonRaw();
        TLorentzVector lep3 = pre_selected_lepton.at(2).getLeptonRaw();
        TLorentzVector lep4 = pre_selected_lepton.at(3).getLeptonRaw();
        dRmin = min(min(min(min(min(lep1.DeltaR(lep2), lep1.DeltaR(lep3)), lep1.DeltaR(lep4)), lep2.DeltaR(lep3)), lep2.DeltaR(lep4)), lep3.DeltaR(lep4));
      }
      plot1D("h_muon_preselected_dRmin", dRmin, 1., h_1d, "pre-selected muon minimum dR", 100, 0, 5.);
...
//      if (std::find(dressing_index.begin(), dressing_index.end(), index)!=dressing_index.end()) continue;
    ...
    for (auto lepton : pre_selected_lepton){
      ...
      TLorentzVector lepton_tlv = lepton.getLeptonRaw();
//      TLorentzVector lepton_tlv = lepton.getLeptonDressed();
    ...
    std::string pass;
    selected_lepton.size()>0 ? pass="passMuon0" : pass="failMuon0";
